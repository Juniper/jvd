...
the whole jvd-aiml-inf-snips.md bundle.

Walks ../evo (this JVD is EVO-only), parses each .conf file's header
(`Topic:`, `Seen on:` EVO device list), and emits a JSON manifest
with per-snip metadata + raw.githubusercontent.com URLs.
...
def _devices(line: str) -> list[str]:
    ...
    Device names always contain an underscore (e.g. leaf1_qfx5130-32cd,
    spine1_qfx5220-32cd). Strip parenthetical phrases like '(none — ...)'.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        "path": rel,
        "os": os_family,            # "evo"
        "category": category,       # bootstrap | interfaces | oam | policy | services | transport
...
def main() -> int:
    ...
    extras = [
        ...
        },
        {
            ...
            "kind": "reference",
            "topic": "Auto-fill rules: lab-default loopbacks, AS numbers, VLAN IDs, /31 links",
