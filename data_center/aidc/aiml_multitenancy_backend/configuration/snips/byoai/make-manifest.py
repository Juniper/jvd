...
the whole jvd-aiml-mtb-snips.md bundle.
...
def _devices(line: str) -> list[str]:
    ...
    Device names always contain an underscore (e.g. leaf1_qfx5240-64od,
    spine1_qfx5240-64od). Strip parenthetical phrases like '(none — ...)'.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        "path": rel,
        "os": os_family,            # "evo"
        "category": category,       # bootstrap | cos | interfaces | oam | policy | services | transport
...
def main() -> int:
    ...
    extras = [
        ...
        },
        {
            ...
            "kind": "reference",
            "topic": "Auto-fill rules: lab-default loopbacks, AS numbers, tenant VNIs, RD/RT, RoCE CoS",
