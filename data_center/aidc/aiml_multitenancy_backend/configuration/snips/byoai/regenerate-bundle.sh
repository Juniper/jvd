#!/usr/bin/env bash
#
# Regenerate jvd-aiml-mtb-snips.md from the current contents of
# snips/evo/ and snips/_variables.md.  This JVD is EVO-only.
#
...
OUT="byoai/jvd-aiml-mtb-snips.md"

{
  echo "# JVD AI/ML Multi-Tenancy Backend (EVPN-VXLAN GPU fabric) snippet library"
  echo
  for f in $(find evo -name '*.conf' | sort); do
    echo "## $f"
    echo
    echo '```'
    cat "$f"
...
PROMPT_OUT="byoai/jvd-aiml-mtb-byoai-prompt.txt"
