...
the whole jvd-so-fwnat-snips.md bundle.

Walks ../junos, parses each .conf file's header (`Topic:`, `Seen on:`
Junos device list), and emits a JSON manifest with per-snip metadata +
...
def _devices(line: str) -> list[str]:
    """Extract device hostnames from a 'Junos:' line.

    Device names always contain an underscore (e.g. mx1_mx304,
    srx1a_srx4600). Strip parenthetical phrases.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    rel = conf_path.relative_to(SNIPS_DIR).as_posix()
    os_family, category, filename = rel.split("/", 2)
    header = parse_header(conf_path)
    return {
        "path": rel,
        "os": os_family,            # "junos"
        "category": category,       # nat | load-balancing | high-availability | security | transport | firewall | interfaces | bootstrap
...
def main() -> int:
    ...
    extras = [
        ...
        },
        {
            "path": "byoai/TIERS.md",
            "kind": "reference",
            "topic": "Per-role snip lists for `minimum` / `with-bgp` / `as-deployed` configuration form",
            "raw_url": f"{RAW_BASE}/byoai/TIERS.md",
        },
        {
            "path": "byoai/DEFAULTS.md",
            "kind": "reference",
            "topic": "Auto-fill rules: lab-default anycast health-check IPs, AS numbers, per-plane /30s, NAPT44 pools, MNHA signal-routes",
