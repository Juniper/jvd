#!/usr/bin/env bash
#
# Regenerate jvd-so-fwnat-snips.md from the current contents of
# snips/junos/ and snips/_variables.md.
#
...
OUT="byoai/jvd-so-fwnat-snips.md"

{
  echo "# JVD Scale-Out Stateful Firewall & NAT (CSDS ScaleOut) snippet library"
  echo
  for f in $(find junos -name '*.conf' | sort); do
    echo "## $f"
    echo
    echo '```'
    cat "$f"
...
PROMPT_OUT="byoai/jvd-so-fwnat-byoai-prompt.txt"
