...
the whole jvd-so-ipsec-snips.md bundle.
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        "path": rel,
        "os": os_family,            # "junos"
        "category": category,       # load-balancing | ipsec | high-availability | security | transport | policy | firewall | interfaces
...
def main() -> int:
    ...
    extras = [
        ...
        },
        {
            ...
            "kind": "reference",
            "topic": "Auto-fill rules: lab-default anycast VIP, AS numbers, per-plane /30s, MNHA signal-routes",
