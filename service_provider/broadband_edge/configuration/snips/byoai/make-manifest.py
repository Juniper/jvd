...
the whole jvd-bbe-snips.md bundle.
...
def _devices(line: str) -> list[str]:
    ...
    Device names always contain an underscore (e.g. bng1_mx304,
    an1_acx7024). Strip parenthetical phrases like '(none — ...)'.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        ...
        "category": category,       # bootstrap | firewall | interfaces | policy | services | subscriber-management | transport
