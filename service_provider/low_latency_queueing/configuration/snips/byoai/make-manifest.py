...
the whole jvd-llq-snips.md bundle.
...
def _devices(line: str) -> list[str]:
    ...
    Device names always contain an underscore (e.g. an4_acx7024,
    sag_mx304). Strip parenthetical phrases like '(none — ...)'.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        ...
        "category": category,       # cos | firewall | interfaces | oam | policy | services | transport
    ...
    extras = [
        {
            "path": "_variables.md",
            "kind": "glossary",
            "topic": "Glossary of <variable> placeholders used across all snips",
