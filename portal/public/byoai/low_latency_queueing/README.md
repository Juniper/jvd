# BYOAI — Bring Your Own AI (Low Latency QoS for 5G / LLQ)

Generate validated Junos / Junos Evolved configuration for the **Low
Latency QoS Design for 5G (LLQ)** JVD using any AI you already have
(Claude, ChatGPT, Gemini, a local model). The AI is grounded in this
JVD's published, validated snippet library — it does not invent syntax.

Two modes:

- **Configuration mode** — a guided interview produces ready-to-deploy
  config from the 70-snip library (EVPN-VPWS/FXC/ELAN, BGP-VPLS, L3VPN,
  and the full 8-queue 5G Class-of-Service model with Low Latency Queuing).
...
| `jvd-llq-byoai-prompt.txt` | Extracted fenced block, standalone |
| `jvd-llq-snips.md` | Full snip bundle (Config-mode corpus) |
...
**Configuration mode** fetches the snip bundle (`jvd-llq-snips.md`) and
generates config. **Design mode** fetches the design corpus and answers
...
This rebuilds `jvd-llq-snips.md`, extracts `jvd-llq-byoai-prompt.txt`,
...
and commit the refreshed `portal/public/byoai/low_latency_queueing/`.
