# BYOAI — Bring Your Own AI (Scale-Out IPsec / CSDS ScaleOut)

Generate validated Junos configuration for the **Scale-Out IPsec**
(Connected Security Distributed Services — CSDS ScaleOut) JVD using any
AI you already have (Claude, ChatGPT, Gemini, a local model). The AI is
grounded in this JVD's published, validated snippet library — it does
not invent syntax.

Two modes:

- **Configuration mode** — a guided interview produces ready-to-deploy
  config from the 23-snip library (the MX RE-based TLB load balancer,
  the SRX responder-only IPsec Security Gateway, SRX MNHA, the
  scale-out BGP planes, and the MX IPsec initiator).
- **Design mode** — explore the architecture, grounded in the JVD
  documentation corpus (`../../../documentation/`). This JVD ships in
  **two deployment focuses** — Enterprise and Mobile Service Provider —
  and Design mode frames answers for whichever you pick.
...
4. It fetches the corpus it needs from GitHub and proceeds. In Design
   ...
   frame for Enterprise or Mobile SP.
...
| `jvd-so-ipsec-byoai-prompt.txt` | Extracted fenced block, standalone |
| `jvd-so-ipsec-snips.md` | Full snip bundle (Config-mode corpus) |
...
**Configuration mode** fetches the snip bundle (`jvd-so-ipsec-snips.md`)
...
- Enterprise: [`design-guide-enterprise.md`](../../../documentation/design-guide-enterprise.md) · [`solution-overview-enterprise.md`](../../../documentation/solution-overview-enterprise.md) · [`test-report-brief-enterprise.md`](../../../documentation/test-report-brief-enterprise.md)
- Mobile SP: [`design-guide-mobile-sp.md`](../../../documentation/design-guide-mobile-sp.md) · [`solution-overview-mobile-sp.md`](../../../documentation/solution-overview-mobile-sp.md) · [`test-report-brief-mobile-sp.md`](../../../documentation/test-report-brief-mobile-sp.md)
...
```bash
./regenerate-bundle.sh   # rebuilds jvd-so-ipsec-snips.md + prompt.txt + MANIFEST.json
```
