...
the whole jvd-ewan-ace-snips.md bundle.
...
def _devices(line: str) -> list[str]:
    ...
    Device names always contain an underscore (e.g. wanedge1_mx304,
    p1_ptx10003). Strip parenthetical phrases like '(none — ...)'.
    """
    line = re.sub(r"\([^)]*\)", "", line)
...
def build_entry(conf_path: Path) -> dict:
    ...
    return {
        ...
        "category": category,       # bootstrap | cos | ddos-mitigation | firewall | interfaces | oam | policy | services | transport
